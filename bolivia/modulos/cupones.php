...
// --- C. ELIMINAR CUPÓN (SOFT DELETE - CAMPO ESTADO A 0) ---
if (isset($_GET['del'])) {
    $id_del = $_GET['del'];
...
// --- D. ANULAR CUPÓN POR CÓDIGO (USADO EN LA VISTA) ---
// NOTA: El campo "usado" NO se toca aquí, solo en el quemado/canje del cupón
...
// Traemos cupones con su tipo de cupón
...
<table style="width:100%; border-collapse: collapse; background:#fff; border-radius:8px; overflow:hidden; box-shadow: 0 4px 15px rgba(0,0,0,0.05);">
    ...
    <tbody>
        ...
        <tr style="border-bottom: 1px solid #f2f2f2; transition: 0.2s;" onmouseover="this.style.background='#fbfbfb'" onmouseout="this.style.background='#fff'">
            ...
            <td>
                <span style="font-weight:bold; color:#000; font-size:14px;"><?php echo $c['tipo_nombre'] ?? 'Sin tipo'; ?></span>
            </td>
            
            <td style="font-size:13px; color:#666;">
                <?php echo $c['descripcion'] ?: '<i style="color:#ccc">Sin descripción</i>'; ?>
            </td>
            ...
            <td style="text-align:center;">
                <?php if($c['estado']): ?>
                    <a>
                    <!--<a href="admin.php?mod=cupones&toggle_estado=<?php echo $c['id']; ?>&estado=<?php echo $c['estado']; ?>" class="btn-pastilla btn-active" title="Click para anular">-->
                        ✅ Activo
                    </a>
                <?php else: ?>
                    <a>
                    <!--<a href="admin.php?mod=cupones&toggle_estado=<?php echo $c['id']; ?>&estado=<?php echo $c['estado']; ?>" class="btn-pastilla btn-inactive" title="Click para activar">-->
                        ⛔ Anulado
                    </a>
                <?php endif; ?>
            </td>
...
<!-- <a href="admin.php?mod=cupones&edit=<?php echo $c['id']; ?>" class="btn-pastilla btn-edit">✏️ Editar</a> -->
        ...
        <form id="formC" method="POST" style="margin-top:25px;">
            ...
            <div class="grid-form">
                ...
                <div class="form-group">
                    ...
                    <small style="color: #666; font-size: 10px; display: block; margin-top: 3px;">
                        Ejemplo: "Vale por 1/4 de pollo" o "Descuento del 8% en 1/8 de pollo"
                    </small>
                </div>
            </div>
                ...
                <?php if(empty($c_edit)): ?>
                    <div class="form-group" style="margin:0;">
                        ...
                        <input type="number" name="cantidad_codigos" min="1" max="1000" value="1" 
                               ...
                               title="Si necesitas múltiples cupones, ingresa la cantidad aquí">
                ...
                <?php else: ?>
                    <div class="form-group" style="margin:0;">
                        ...
                        <div class="codigo-card">
                            <strong><?php echo $c_edit['codigo']; ?></strong>
                        </div>
...
<script>
    ...
    function abrirModalNuevo() {
        if(estaEditando) {
            window.location.href = 'admin.php?mod=cupones'; 
            return;
        }
    ...
    function abrirConfirmar() {
        ...
        if(tipo.value === "") {
            alert('Por favor selecciona un tipo de cupón.');
            tipo.focus();
            return;
        }
        
        if(!document.getElementById('formC').checkValidity()) {
            document.getElementById('formC').reportValidity();
            return;
        }
        ...
        let text = "";
        if(esMasivo) {
            text = `¿Deseas generar ${cantidad.value} cupones del tipo "${nombreTipo}"?`;
        } else if(id) {
            text = `¿Estás seguro de guardar los cambios del cupón?`;
        } else {
            text = `¿Deseas crear un nuevo cupón del tipo "${nombreTipo}"?`;
        }
